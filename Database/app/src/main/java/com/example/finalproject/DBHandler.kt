...
class DBHandler (context: Context, name: String?,
    ...
    fun updateWeightTrack(id: Int, weight: Int, date: String)
    {
        val values = ContentValues()
        //inserting the values the user wants to change
        values.put(COLUMN_ID, id)
        values.put(COLUMN_WEIGHT, weight)
        values.put(COLUMN_DATE, date)
        //deleting the values at id
        deleteWeightTrack(id)
        val db = this.writableDatabase
        //inserting the new values into the database at the id the user wants
        db.insert(TABLE_WEIGHTTRACK,null, values)
        db.close()
